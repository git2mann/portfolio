...
export default async function TechBlogPage({
  ...
  return (
    <main className="min-h-screen bg-[#EAE8E3] text-black selection:bg-[#7C3AED] selection:text-white">
         ...
         <Container>
            <div className="py-12 md:py-24 grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-12">
               <div>
                  <Link 
                     href="/blog" 
                     className="inline-flex items-center gap-2 font-mono text-xs uppercase tracking-widest border-b-2 border-black pb-1 hover:text-[#7C3AED] hover:border-[#7C3AED] transition-colors mb-8"
                  >
                  ...
                  <h1 className="text-6xl md:text-8xl font-black uppercase leading-[0.85] tracking-tighter mb-6 relative z-10">
                     Tech<br/>
                     <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#7C3AED] to-[#FF3B30]">Logs</span>
                  </h1>
                  <p className="text-lg font-medium border-l-4 border-[#7C3AED] pl-6 max-w-lg leading-relaxed">
                     Code repositories, system architecture, and development insights.
                  </p>
               </div>
               
               {/* Decorative Abstract Visual */}
               <div className="hidden lg:flex items-center justify-center relative">
                  <div className="w-64 h-64 border-4 border-black bg-white shadow-[16px_16px_0px_0px_rgba(0,0,0,1)] flex items-center justify-center relative group overflow-hidden">
                     <div className="absolute inset-0 bg-[#7C3AED] opacity-0 group-hover:opacity-20 transition-opacity"></div>
                     <Terminal className="w-24 h-24 text-black group-hover:text-[#7C3AED] transition-colors" />
                     <div className="absolute bottom-4 right-4 font-mono text-xs font-bold uppercase tracking-widest bg-black text-white px-2 py-1">
                        /bin/bash
                     </div>
                  </div>
               </div>
        ...
        <div className="max-w-[1920px] mx-auto py-12 relative z-10">
                ...
                <Link
                  ...
                  className={`
                    ...
                    ${!tag 
                      ...
                      : "bg-transparent text-black border-black hover:bg-[#7C3AED] hover:text-white hover:border-[#7C3AED]"
                    }
                  `}
                >
                  [ ALL_SYSTEMS ]
            ...
            <div className="mb-24 group relative">
               ...
               <Link href={`/posts/${featuredPost.slug}`} className="block border-4 border-black bg-white relative z-10">
                  <div className="grid grid-cols-1 lg:grid-cols-2">
                     <div className="relative min-h-[400px] border-b-4 lg:border-b-0 lg:border-r-4 border-black overflow-hidden bg-black">
                        <Image
                     ...
                     <div className="p-8 lg:p-12 flex flex-col justify-center">
                        <div className="flex items-center gap-2 mb-4">
                           <span className="bg-[#7C3AED] text-white px-3 py-1 font-mono text-[10px] uppercase tracking-widest border border-black">
                              System_Alert
                           </span>
                           {featuredPost.date && (
                        ...
                        <h3 className="text-4xl lg:text-5xl font-black uppercase leading-[0.9] tracking-tighter mb-6 group-hover:text-[#7C3AED] transition-colors">
                           {featuredPost.title}
                        </h3>
                        ...
                        <div className="flex items-center gap-2 font-bold uppercase text-sm tracking-widest group-hover:gap-4 transition-all">
                           Execute File <ArrowRight className="w-4 h-4" />
                        </div>
                     </div>
                  </div>
               </Link>
              ...
              >
                <Link href={`/posts/${post.slug}`} className="block flex-1 flex flex-col">
                   ...
                   <div className="p-6 flex flex-col flex-1 bg-white">
                      <time className="font-mono text-xs text-[#7C3AED] font-bold tracking-widest mb-3 block">
                         {new Date(post.date).toLocaleDateString()}
                      </time>
                      
                      <h3 className="text-2xl font-black leading-tight mb-4 uppercase tracking-tight group-hover:text-[#7C3AED] transition-colors">
                         {post.title}
                      </h3>
          ...
          {paginatedPosts.length === 0 && !featuredPost && (
            <div className="border-4 border-black border-dashed p-16 text-center bg-white">
               <div className="w-16 h-16 bg-[#7C3AED] mx-auto mb-6 flex items-center justify-center border-2 border-black rounded-full">
               ...
               <Link 
                  href="/blog/tech" 
                  className="inline-block px-8 py-3 bg-black text-white font-bold uppercase tracking-widest hover:bg-[#7C3AED] transition-colors"
               >
                  Reset Filters
               </Link>
            </div>
               ...
               <div className="inline-flex border-2 border-black bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                  <Link
                     ...
                     className={`px-6 py-3 font-bold uppercase tracking-widest border-r-2 border-black transition-colors ${
                        currentPage === 1 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'hover:bg-[#7C3AED] hover:text-white'
                     }`}
                  ...
                  <Link
                     ...
                     className={`px-6 py-3 font-bold uppercase tracking-widest border-l-2 border-black transition-colors ${
                        currentPage === totalPages ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'hover:bg-[#7C3AED] hover:text-white'
                     }`}
