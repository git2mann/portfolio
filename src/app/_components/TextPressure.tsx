...
const TextPressure = ({
  ...
  useEffect(() => {
    ...
    const animate = () => {
      if (mouseMode) {
        ...
        // Automatic motion using a very slow, occasional sine wave
        if (containerRef.current) {
          const { left, top, width, height } = containerRef.current.getBoundingClientRect();
          const elapsed = (Date.now() - startTime) / 1000;
          
          // Much slower frequency (0.3) and higher power (4) for a rare, brief ripple
          const rawSine = Math.sin(elapsed * 0.5);
          const t = (Math.pow(Math.abs(rawSine), 4) * (rawSine < 0 ? -1 : 1) + 1) / 2;
          
          mouseRef.current.x = left + (width * t);
          mouseRef.current.y = top + height / 2;
        }
      }
  ...
  }, [mouseMode, width, weight, italic, alpha, minWght, maxWght, minWdth, maxWdth]);

  const styleElement = useMemo(() => {
    // Only inject style if fontUrl is provided
