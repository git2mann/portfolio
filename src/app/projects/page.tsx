  ...
  return (
    <main className="min-h-screen pb-32 bg-background-primary">
      ...
      <div id="projects" className="sticky top-16 md:top-24 z-40 mb-12 md:mb-20 pt-6 md:pt-10">
        <div className="max-w-fit mx-auto liquid-glass px-2 py-2 rounded-full shadow-lg border border-primary/10">
      ...
      <Container className="!max-w-none px-6 md:px-20">
        <section className="animate-in fade-in slide-in-from-bottom-8 duration-700">
          <div className="flex flex-col md:flex-row items-start md:items-end justify-between mb-12 md:mb-20 gap-8 md:gap-12 border-l-2 border-accent-blue/20 pl-8 md:pl-12">
             <div>
                <div className="flex items-center gap-4 mb-4 md:mb-6">
                    <span className="text-accent-blue font-medium text-xs md:text-sm uppercase tracking-[0.4em]">Section 01</span>
                    <span className="text-secondary opacity-30 font-mono text-xs md:text-sm">REPOSITORY</span>
                </div>
                <h2 className="text-4xl md:text-9xl font-light uppercase tracking-tighter leading-tight md:leading-tight text-primary">Coming Soon</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 mb-32">
            <div className="group relative flex flex-col bg-primary/[0.01] border border-primary/10 p-10 md:p-20 rounded-sm overflow-hidden">
               <div className="absolute top-6 right-8 font-mono text-xs md:text-sm opacity-60 uppercase tracking-widest pointer-events-none text-accent-blue font-bold">Status: In Progress</div>
               <div className="relative z-10">
                  <div className="w-12 h-12 md:w-16 md:h-16 rounded-xl bg-accent-blue/10 text-accent-blue mb-8 md:mb-12 flex items-center justify-center border border-accent-blue/20">
                     <Activity size={24} />
                  </div>
                  <h3 className="text-4xl md:text-7xl font-light mb-6 md:mb-10 uppercase tracking-tight text-primary leading-snug md:leading-snug">Development In Progress</h3>
                  ...
                  <div className="flex flex-col sm:flex-row gap-6 md:gap-12">
                     <Link 
                        href="https://github.com/git2mann" 
                        target="_blank"
                        className="group/btn flex items-center gap-4 text-sm md:text-lg font-medium uppercase tracking-[0.3em] pb-2 border-b-2 border-accent-blue/40 hover:border-accent-blue transition-all hover:gap-8 text-primary"
                     >
                        GitHub Profile <ExternalLink size={20} />
                     </Link>
                     <Link href="/contact" className="group/btn flex items-center gap-4 text-sm md:text-lg font-medium uppercase tracking-[0.3em] pb-2 border-b-2 border-primary/20 hover:border-primary transition-all hover:gap-8 text-primary">
                        Inquire <ArrowRight size={20} />
                     </Link>
                  </div>
               </div>
               <div className="mt-16 md:mt-24 h-px w-full bg-primary/10 relative overflow-hidden">
                  <motion.div 
                    animate={{ x: ["-100%", "200%"] }} 
                    transition={{ duration: 4, repeat: Infinity, ease: "linear" }}
                    className="absolute inset-0 h-full w-1/4 bg-gradient-to-r from-transparent via-accent-blue/40 to-transparent"
                  />
               </div>
            </div>
            ...
            <div className="hidden md:flex flex-col justify-center gap-6 opacity-10 pointer-events-none">
               {[1, 2, 3].map(i => (
                 <div key={i} className="h-24 md:h-32 border border-primary/10 rounded-sm flex items-center px-12 justify-between font-mono text-sm md:text-base uppercase tracking-widest text-primary/45">
